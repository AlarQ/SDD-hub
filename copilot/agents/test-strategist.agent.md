...
description: Designs cross-task test strategies to prevent duplication, assign test ownership, and ensure integration coverage. Used by /propose after task generation and /implement before test writing.
handoffs:
  - label: "Review Test Strategy"
    agent: "agent"
    prompt: "Run /propose for this feature to see the test strategy."
    send: false
...
When invoked from `/propose` after task generation, return:
...
When invoked from `/implement` before test writing, return:
