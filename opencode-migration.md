...
3. **20 existing subagents port as-is** — already `mode: subagent` shape (Karen, config-inferencer, software-architect, code-reviewer, ultrathink-debugger, etc.). SpecDriver invokes via task tool. Parallel dispatch (validate Phase 2) happens inside SpecDriver, not from command body.
