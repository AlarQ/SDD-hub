...
- `commands/*.md` — Slash command definitions (bootstrap, config, explore, propose, implement, validate, validate-impl, review-and-ship, learn-from-reports, quick-ship, pr-review, fix, spec-status, workflow-summary, continue-task, research, promote-tier, capture-rule). The deterministic commit/push/PR-ready logic lives in `scripts/ship-procedure.md` (a shared prose procedure run inline by `/validate` and `/review-and-ship` — there is no `/ship` command).
...
- Explicit per-step invocation: each command is invoked separately by the user. After finishing, every command prints the next command to run (e.g. `/implement` → "Draft PR opened. Run `/pr-review $ARGUMENTS` after commenting, or `/validate $ARGUMENTS` if no comments."). No command auto-invokes another — shipping is now a shared **procedure** (`scripts/ship-procedure.md`) inlined by its caller (same mechanism as inlining `multi-repo-resolution.md` / `pr-body-convention.md`), not a slash command, so this rule still holds. Sequence per task: `/implement` (opens draft PR) → `/pr-review` (loop until PR comments resolved; optional if none) → `/validate` (zero findings → ships inline, marks PR ready) → (`/review-and-ship` if findings — addresses them, then ships inline) → `/learn-from-reports` (final manual step; task already shipped) → merge PR → `/implement` (next task). Reaching `done` now coincides with shipping (PR ready). After the last task transitions to `done`, the user runs `/validate-impl` for the final spec-completion audit.
...
The feedback loop (`/review-and-ship`, `/learn-from-reports`, `/capture-rule`)
...
**`/validate` Phase 3 (per-task coverage audit):** After Phase-2 agent gates, reuses **Odium** (`agents/odium.md`, not edited) to verify the task diff covers the task's own acceptance criteria (`## Acceptance` + `## Implements` FR refs on feature track; `technical_acceptance:` on technical track). Advisory — gaps become `source: llm` findings in `reports/<task-id>-coverage.yaml` flowing through the normal `/review-and-ship` pipeline; no new hard-block. Skipped (first match) when: `WF_SPEC_TIER == small` (`tier_small`), `WF_COVERAGE_AUDIT == false` (`config_off`), `WF_VALIDATE_SCOPE == per-spec` (`scope=per-spec`). Emits `coverage_audit_start`, `coverage_audit_done` monitor events; skips emit `gate_skip` (`gate: coverage`).

**`/validate-impl`:** Runs once when all spec tasks reach `done` (each task already shipped at its own `/validate`/`/review-and-ship`). Spawns Odium with spec FR list, prd.md scope, task list, and git diff range. Verdict `complete` → spec done; verdict `reopen` → `/review-and-ship` spawns follow-up tasks (spec-level triage — no task in `review`, so its ship tail is a no-op).
...
|------|--------------|---------------|---------------|------------|
| `small`  | 2–4         | 5             | 10            | `/explore` (incl. Step −1 grill pass) → `/propose` (tasks/ only — skip spec.md, design.md, test-strategy.md) → `/implement` → `/validate` (lint+tests only; skip Phase-2 agent gates per `WF_TIER_AGENT_SKIP`; ships inline on pass). Skip `/validate-impl` Odium audit. |
...
- `single-branch` = one branch for tightly-coupled refactors/spikes where
  ...
  opened/readied at the **final** task's inline ship (last task), base `main`. TDD
...
- `/pr-review` mid-spec on `single-branch` is a clean dead-end: with no PR open
  ...
  spec PR opened at the final task's inline ship).
...
- Loader exports `WF_BRANCH_STRATEGY` (see `scripts/config-loader.contract.md`);
  included in the config snapshot so the shared ship procedure's drift check catches a mid-spec flip.
...
`/implement`, `/validate`, `/review-and-ship`, `/fix`, `/quick-ship` resolve `WF_TASK_REPO_PATH` per `scripts/multi-repo-resolution.md` and run git/gates/PR creation against that path (the shared ship procedure inherits the already-resolved path from its `/validate` / `/review-and-ship` caller). (`/pr-review` is the exception — it carries no multi-repo resolution and operates on the current branch's PR via `gh` in the working directory.) `/validate-impl` emits per-repo diff sections for Odium. Gates may declare `applies_to_repos: [<name>,…]` to scope.
...
- **Ship is a shared inline procedure, not a command (`scripts/ship-procedure.md`)** — the deterministic commit/push/PR-ready logic runs inline at the two terminal points of the per-task flow: `/validate`'s zero-findings PASS path, and `/review-and-ship`'s tail (only when the current task is at status `review`). Reaching `done` coincides with shipping. `/fix` is the exception — it ships via `/quick-ship`. There is no `/ship` command.
...
- `/validate` Phase 3 runs a per-task **coverage audit** (reuses Odium) — advisory finding when the diff misses the task's own acceptance criteria; gated by `coverage_audit`/tier/scope (see "Configurable Workflow"). Report `reports/<task-id>-coverage.yaml` is just another gate report
- Agent findings are advisory (`source: llm`), tool findings are high-confidence (`source: tool`); both go through `/review-and-ship`
...
- `/propose` ends with a `Spec Reviewer` subagent (`engineering-spec-reviewer`) consistency pass on every tier and track — audits doc↔doc consistency, FR→task traceability, task-graph sanity, and repo alignment; writes `specs/<feature>/reports/spec-consistency.yaml`. Findings hard-block `/propose` from returning success; resolved via `/review-and-ship` (spec-level triage — no task in `review`, ship tail is a no-op). `/propose` does NOT re-run the subagent after review — once findings are resolved the user runs `/implement` directly. `setup.sh` prunes orphan global files (per-file `yes` confirmation; `--no-prune` to skip) so removed agents/commands don't linger in `~/.claude/`.
...
- **Chunked delegated implementation (ADR-0018, extends ADR-0004)**: on the delegated path only, the loop is spread across **bounded contexts** to cap *peak* per-agent context (the driver is doc+diff accumulation under whole-task ownership). When `WF_SPEC_TIER != small` **and** `K = WF_IMPL_CHUNK_SIZE > 0` **and** backlog length > K, main cuts the settled *ordered* backlog into runs of ≤K whole behaviors (mechanical — no planner; cuts between behaviors, never splitting a test from its code) and re-spawns the **same** `implementer:` per chunk with fresh context, threading a cumulative `impl_notes` **ledger** forward. Each chunk runs the **full** suite (all prior behaviors stay green) and refactors locally; the **final** chunk performs the closing whole-diff refactor (whole-task diff, same branch-strategy-aware range as the post-impl quality check, + ledger). `blocked` on any chunk stops the loop (task stays `in-progress`; prior chunks' green tests on disk are recoverable — `/continue-task` re-runs the suite and re-chunks the remainder). Step 12 writes the **merged ledger** prefixed with `chunks_spawned: N` (a persisted note-field, **not** a monitor event — no new telemetry). `impl_chunk_size` is a per-tier default (`small: 0`=off / `medium`,`large: 3`) with per-spec `config.yml impl_chunk_size` override; loader exports `WF_IMPL_CHUNK_SIZE` (`small` forced to `0`). Thin tasks (backlog ≤ K) → one chunk = exact ADR-0004 delegated behavior. Full re-read of design/ground-rule paths per chunk (peak-vs-total token trade, accepted). Inline/`generalist` path untouched.
...
- `/learn-from-reports` is the **final manual step** of the per-task flow — it runs **after the task is already shipped** (after `/review-and-ship` ships, or after `/validate` passes clean and ships) and mines reports for cross-finding patterns — recurring categories, clustered LLM findings, rejection reasoning, generalizable accepted fixes — proposing new general-KB rules in a single batched review. It writes KB rules only and never touches the task PR diff. Reports are retained after mining (local audit trail) — never deleted; mining is scoped per-task by task-id (`/learn-from-reports <feature> <task-id>`, fallback to newest per-task report when arg absent). Complements inline rule creation in `/review-and-ship` (which catches one-off rules) by catching patterns that span findings.
- **PR-first review loop**: `/implement` opens a **draft PR** immediately after marking the task `implemented` so the user can review the diff on GitHub before validation. `/pr-review` resolves the current branch's PR (or a PR number arg), reads its comments, classifies each as `informational` (answer/explain — no code) or `change` (apply a code edit), and walks a per-comment confirm loop (`Apply`/`Re-classify`/`Skip`). It posts threaded `[claude]` replies — informational comments get answers grounded in code; change comments get applied, committed (`pr-review: address comment <id>`), pushed, and replied with the resulting short-sha + what/how. It skips its own `[claude]` replies; there is no reaction-based dedup, so re-runs re-surface still-open comments (use `Skip`). It carries no spec-workflow integration (no config-loader, branch-strategy, multi-repo, or monitor events). The inline ship procedure (run by `/validate` pass / `/review-and-ship`) then marks the existing draft PR ready-for-review via `gh pr ready` instead of creating a new PR.
- **PR-body convention**: PR bodies (`/implement` draft, the shared ship procedure's ready + single-branch spec PR, `/quick-ship`) follow `scripts/pr-body-convention.md` — a `## Why` (1–2 sentences) + `## What changed` (3–6 high-level bullets, no file enumeration) skeleton plus a conditional mermaid diagram (only when control flow / a state machine / a command sequence changes; diagram-type heuristics reused from `skills/propose/SKILL.md`). One shared doc, command-owned footers preserved.
