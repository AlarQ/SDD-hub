...
- **Refactor scope**:
  - **Final chunk only** (the chunk whose completion makes the whole backlog green): "After your slice loop is green, perform the closing whole-diff refactor — read `git -C <WF_TASK_REPO_PATH> diff <task_base_sha>..HEAD` plus the ledger, then extract duplication / deepen modules across the entire task diff, keeping the full suite green."
