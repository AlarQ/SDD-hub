...
3. Identify the task: extract task ID from the current branch name (`feat/$ARGUMENTS/{task-id}-{task-name}`) and read the matching task file from `specs/$ARGUMENTS/tasks/`
...
4. Read the task's `ground_rules` to know which knowledge-base rules apply, resolving prefixes: `general:` → `~/.claude/knowledge-base/`, `project:` → `knowledge-base/`, unprefixed → `project:`
