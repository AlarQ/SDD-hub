...
Loader contract (env vars + exit codes): `scripts/config-loader.contract.md`. This step uses `WF_SPEC_CONFIG_FILE` and `WF_SPEC_GATES`. On non-zero, halt and print loader error; exit-code 4 → run `/explore` or `/config $ARGUMENTS`.
