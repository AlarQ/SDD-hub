---
id: "014"
name: "/validate-spec command + Karen wrapper prompt"
status: blocked
blocked_by: ["013"]
max_files: 4
estimated_files:
  ...
  - scripts/monitor.sh
  - tests/test-validate-spec.sh
  - tests/fixtures/spec-audit/sample-spec/
test_cases:
  - "/validate-spec sources config-loader --spec <feature> and reads WF_VALIDATE_SCOPE"
  - "/validate-spec parses ### FR-N: headings from spec.md and builds an FR id list"
  ...
  - "/validate-spec writes specs/<feature>/reports/spec-audit-<ISO8601>.md with frontmatter {feature, timestamp, scope, verdict}"
...
New slash command `/validate-spec <feature>` that runs the spec-completion audit. Reuses existing Karen agent (`agents/karen.md`) unchanged per ADR-008 — all spec-specific context flows through the wrapper prompt built in this command.

## Public API

- `commands/validate-spec.md` — new command definition. Takes `$ARGUMENTS = feature`. Steps per FR-15:
...
- Karen wrapper prompt is the only specialization surface. Keep it as a Markdown block in `commands/validate-spec.md` so it's versioned alongside the command.
...
- Gate execution (union mode) is NOT in this task — T016 owns scope-dependent gate invocation. `/validate-spec` calls into T016's helper once it exists.
