---
id: "014"
name: "/validate-impl command + Odium wrapper prompt"
status: done
blocked_by: []
max_files: 4
estimated_files:
...
test_cases:
  ...
  - "Odium wrapper prompt contains spec.md FR list, prd.md scope, task list, report paths, git diff range"
  - "Odium wrapper prompt instructs FR × status matrix output with {implemented, partial, missing} enum"
  ...
  - "Odium agent is spawned via Agent tool with subagent_type=odium (no agent definition edits)"
  - "Union gate failure path: blocking gate exit non-zero forces audit verdict=reopen AND Odium still spawned with failing-gate output embedded in wrapper prompt as additional evidence"
...
New slash command `/validate-impl <feature>` that runs the spec-completion audit. Reuses existing Odium agent (`agents/odium.md`) unchanged per ADR-008 — all spec-specific context flows through the wrapper prompt built in this command.
...
- `commands/validate-impl.md` — new command definition. Takes `$ARGUMENTS = feature`. Steps per FR-15:
  ...
  3. Build Odium wrapper prompt with: FR list, PRD scope block, task frontmatter list, `specs/<feature>/reports/` contents, git diff range from branch-point to HEAD.
  4. Spawn Odium via the Agent tool. Instruct: produce Markdown with frontmatter + FR matrix section + orphan-code section + over-engineering flags section.
...
- Odium wrapper prompt is the only specialization surface. Keep it as a Markdown block in `commands/validate-impl.md` so it's versioned alongside the command.
...
- Fixture spec under `tests/fixtures/spec-audit/sample-spec/` with 3 FRs and 2 tasks — used for end-to-end smoke without hitting real Odium; stub agent invocation to return a canned report for tests.

## Implementation Notes (T014)

- Odium invocation is the slash-command's responsibility (Agent tool, `subagent_type: odium`); shell helpers in `scripts/validate-impl.sh` cover everything else and are unit-testable without spawning the agent.
...
- T016 owns the union-gate executor; `commands/validate-impl.md` Step 2 is documented but inert until that helper lands. Blocking-gate failure forces `verdict=reopen` AND still spawns Odium with failing output as additional evidence — `wf_vi_build_prompt` accepts an `extra_evidence` file argument so the contract is exercised today (`test_build_prompt_includes_extra_evidence`).
