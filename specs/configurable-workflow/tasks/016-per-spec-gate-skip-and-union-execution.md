---
id: "016"
name: "Per-spec gate skip + union execution at /validate-spec"
status: blocked
blocked_by: ["013", "004"]
max_files: 4
estimated_files:
...
test_cases:
  ...
  - "/validate under scope=both runs full intersection AND allows /validate-spec to run later"
  - "/validate-spec executes union of spec-eligible gates (not per-task intersection) against cumulative diff"
...
Make scope semantics real. `/validate` honors `WF_VALIDATE_SCOPE` and short-circuits per-task execution when `per-spec` is chosen. `/validate-spec` computes the union of spec-eligible gates (FR-2 `applies_to` filter ∩ FR-3 spec ceiling) and runs each once against the cumulative diff.
...
- `commands/validate-spec.md` — new step (inserted before Karen spawn per FR-15 step 2): if `scope ∈ {per-spec, both}`, compute union `{g | g ∈ WF_SPEC_GATES ∧ g.applies_to ∩ union(task.ground_rules_languages) ≠ ∅}`, execute each gate once against the cumulative diff range. Results are part of the audit report input.
...
- Empty-intersection fail-closed rule (ADR-003) still applies: if the union is empty AND at least one task is code-bearing, `/validate-spec` fails closed before spawning Karen.
