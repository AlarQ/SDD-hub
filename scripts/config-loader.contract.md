...
| `WF_BRANCH_STRATEGY` | enum | `--spec` only | `per-task` (default) \| `single-branch`. Optional in `config.yml`; absence → `per-task`. `per-task` = per-task sub-branch + per-task draft PR (current behavior). `single-branch` = one `feat/$FEATURE` branch off main, commits accumulate, no per-task PR, one spec PR opened/readied at the final `/ship` (base `main`). Invalid value → exit 4. Included in the config snapshot — a mid-spec flip is caught by `/ship` drift check. |
