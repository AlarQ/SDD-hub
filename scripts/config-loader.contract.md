...
| `WF_IMPL_CHUNK_SIZE` | int | `--spec` only | Behavior-count budget **K** for chunked delegated `/implement` (ADR-0018): main cuts a delegated task's settled behavior backlog into ordered ≤K chunks and re-spawns the same `implementer:` per chunk with fresh context. Resolution: per-spec `config.yml impl_chunk_size` override → `.workflow.yml tiers.<tier>.impl_chunk_size` default → hardcoded `3`. `small` tier is **forced to `0`** (never chunks) regardless of override/default. `0` = chunking off (one delegated spawn = pre-ADR-0018 behavior). Non-negative integer; invalid → exit 4. Only affects the delegated path — inline/`generalist` ignores it. |
