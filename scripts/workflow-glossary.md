...
- **repo mode** — `spec_storage_mode: repo` (default): specs live inside the code repo; one `gate_pool` + `knowledge-base/` resolved from `.workflow.yml`.
...
- **thin pointer** — a vault `.workflow.yml` carrying only workflow settings + `general_kb_path`; declares no `gate_pool`, ships no `knowledge-base/`.
- **bound repo** — a code repo listed in a spec's `config.yml repos[]` (`name → path → role`). Owns its own `gates.yml` + `knowledge-base/`.
- **implicit default repo** — when a spec binds exactly one repo, bare `project:`/unprefixed ground rules resolve to that repo's `knowledge-base/`. Two+ repos → strict `repo:<name>:` required.
...
- **gate-id union (vault)** — in vault mode a spec's `gates:` are validated against the union of all bound repos' `gates.yml` ids (no single pool exists).
